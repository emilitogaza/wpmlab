  ...
  return (
    <div className="flex w-full max-w-4xl flex-col gap-8">
      <div className="flex items-baseline justify-between gap-4">
        ...
        <span className="text-ink-faint text-sm">{describeConfig(snapshot.config)}</span>
      </div>
      ...
      {stillGoing && <p className="text-ink-faint text-sm">still racing — this fills in as they finish</p>}
      ...
      <div className="flex justify-center">
        {canRematch ? (
          ...
          <p className="text-ink-dim text-sm">when they go again, you&apos;re in — hang tight</p>
        )}
      </div>
    </div>
  );
...
function Row({ player, position, isMe }: { player: RacePlayer; position: number; isMe: boolean }) {
  ...
  return (
    ...
    >
      <span className="text-ink-faint text-sm">{position}</span>

      <span className="flex min-w-0 items-center gap-2">
        <span className={cn("size-2.5 shrink-0 rounded-full", raceColorBg(player.colorIndex))} />
        <span className={cn("truncate text-sm", raceColorText(player.colorIndex))}>{player.name}</span>
        {isMe && <span className="text-ink-faint text-xs">you</span>}
      </span>
...
function Cell({ label, value, strong }: { label: string; value: string | number; strong?: boolean }) {
  return (
    <span className="hidden text-right sm:block">
      <span className="block text-[0.625rem] text-ink-faint">{label}</span>
      <span className={cn("block", strong ? "text-ink text-lg" : "text-ink-dim text-sm")}>{value}</span>
    </span>
  );
}
